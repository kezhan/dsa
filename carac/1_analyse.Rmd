...
On peut créer une boucle pour importer les fichier efficacement.
...
On peut afficher un résumé statistique des données.
...
```{r}
mono19[,.(.N,PM_ALM=sum(PM_ALM)),by=CodeProduit]
```
...
```{r}
mono19[,.N,by=AnneeFinGarantiePourTMG]
...
mono19[,.N,by=c("AnneeFinGarantiePourTMG","TauxChargementSurEncours")]%>%
  spread(TauxChargementSurEncours,N)
```
